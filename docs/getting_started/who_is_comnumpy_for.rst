...
4. **OptiCommPy**:
   ...
   - **Cross-validated against it**: ``validation/optical_wdm_opticommpy.py`` reproduces the published 11 |times| 32 GBd PDM-16QAM, 700 km example of OptiCommPy, down to their pulse shaping: the same root-raised-cosine, 1024 taps at 16 samples per symbol, and the same filter object at both ends of the link, as they do. The transmitted comb power matches their printed figure to 0.004 dB, the achievable rates (MI, GMI, NGMI) match theirs exactly, and their end-to-end SNR sits inside the bracket this reproduction gives it: 21.69 dB with a converged split step and no receiver impairments, 20.18 dB through a cruder receiver than theirs, against their 20.63 dB. The gap is the receiver, and the script shows it rather than asserting it -- refining the split step *widens* it, adding phase noise, a polarization rotation, a CMA/RDE butterfly and a blind phase search closes it. The same run measures where they are ahead: on a *fixed* split-step grid, their nominal 0.5 km step is 0.85 dB short of converged, and it is their adaptive stepping that makes it enough.
