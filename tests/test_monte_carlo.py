"""monte_carlo(): reconfigure, reseed, run, collect (decision D35)."""
import unittest

import numpy as np

from comnumpy import AWGN, Sequential, SymbolDemapper, \
    SymbolGenerator, SymbolMapper, compute_ser, get_alphabet, monte_carlo
...
class TestMonteCarlo(unittest.TestCase):
    ...
    def test_results_aligned_and_monotonic(self):
        ...
        out = monte_carlo(self.build(), "noise.snr_dB", values,
    ...
    def test_reproducible_across_calls(self):
        values = [0, 6]
        a = monte_carlo(self.build(), "noise.snr_dB", values,
                        {"ser": compute_ser}, 5_000, reference="tx", seed=3)
        b = monte_carlo(self.build(), "noise.snr_dB", values,
                        {"ser": compute_ser}, 5_000, reference="tx", seed=3)
        np.testing.assert_array_equal(a["ser"], b["ser"])

    def test_multi_param_zip(self):
        chain = self.build()
        out = monte_carlo(chain, ["noise.snr_dB", "noise.name"],
                          [(0, "noise"), (10, "noise")],
    ...
    def test_metric_without_reference(self):
        out = monte_carlo(self.build(), "noise.snr_dB", [0, 20],
