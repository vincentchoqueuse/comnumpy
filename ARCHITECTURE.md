...
*monitors* sont supprimés, non convertis en loggers) ; mise à jour de la
cible d'usage de **D40c** et du point ouvert « graphe de calcul » de la
§8, dont D42 traite le cas de la seconde entrée.
...
| # | Décision | Motif | Alternatives rejetées | Statut |
|---|----------|-------|-----------------------|--------|
| D22 | **Cycle de vie inspiré de scikit-learn.** Signature unique `fit(X, y=None)` où `y` est la référence connue (préambule, pilotes) ; `y=None` = estimation aveugle. `fit` retourne `self`. `partial_fit(X)` pour les algorithmes adaptatifs (CMA, RDE, décision-dirigé). `forward()` reste l'interface unique du `Processor` | La convention sklearn est la plus répandue en Python scientifique, et le couple supervisé/non-supervisé se superpose exactement à *data-aided*/*blind*. L'audit montre que `fit()` existe déjà sur 8 classes, mais que le second argument y désigne tour à tour `x_target`, `x_preamble` et `w0` (une initialisation d'algorithme, pas une donnée) | Renommer `forward` en `transform` (fragmenterait `Sequential`) ; garder l'état actuel | **Acté** |
...
chain = Sequential([
    ...
    TrainedBasedPhaseCompensator(target_data=np.zeros(1), name="phase"),
    SymbolDemapper(alphabet),
], taps=["data_tx", "awgn", "gsop", "phase"],
   wiring={"phase.target_data": "signal_tx"})
...
- *Graphe de calcul* — **frontière de périmètre.** `Sequential.forward`
  ...
  déjà : `TrainedBasedComplexGainCompensator` recevait `extractor` et
  `target_data` **par le constructeur**, faute de pouvoir les recevoir
