...
class FIR_CD_Compensator(Processor):
    """
    :param z: length in km
    :param lamb: wavelength [m]
    :param D: fiber dispersion parameter [s/m/m]
    :param c: speed of light [km/s]
    """

    def __init__(self, z, D=17*(10**-6), lamb=1553*(10**-9), F_s=1, c=3*(10**8), name="savory"):
        self.F_s = F_s
        self.z = z  # in m
        self.lamb = lamb # in m
        self.D = D # in s/m/m
...
class LS_FIR_CD_Compensator(FIR_CD_Compensator):
    ...
    :param z: length in km
    :param lamb: wavelength [m]
    :param D: fiber dispersion parameter [ps/nm/km]
    :param c: speed of light [km/s]
    ...
    * Optimal Least-Squares FIR Digital Filters for Compensation of Chromatic Dispersion in Digital Coherent Optical Receivers
    """
    def __init__(self, z, N, D=17*(10**-6), lamb=1553*(10**-9), F_s=1, c=3*(10**8), epsilon=10**(-14), w_vect = [-np.pi,np.pi], name="optimal"):
        self.F_s = F_s
        self.z = z  # in m
        self.lamb = lamb  # in m
        self.D = D  # in ps/m/m
