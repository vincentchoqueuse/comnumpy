...
class Laser(Processor):
    ...
    theta0 : float, optional, keyword-only
        Initial phase :math:`\theta_0` in rad. When None, drawn
        uniformly on :math:`[-\pi, \pi)` from the local generator.
    ...
    def forward(self, nb_samples: object) -> np.ndarray:
        ...
        if not isinstance(self.theta0, (float, int)):
            # an optical phase with no prior is uniform on [-pi, pi);
            # the Gaussian this replaced had std 2*pi and no wrapping
            theta_0 = float(self.rng.uniform(-np.pi, np.pi))
        else:
            theta_0 = self.theta0
...
class Optical90HybridCircuit(Processor):
    ...
    Axes: *element-wise* -- applied pointwise; one local-oscillator
    draw per array element, whatever the shape.
...
class ErbiumDopedFiberAmplifier(Processor):
    ...
    Axes: *element-wise* -- pointwise gain plus additive noise; one
    independent complex ASE draw per array element, whatever the shape
    -- a polarization pair ``(..., 2, N)`` gets independent noise per
    polarization.
...
class MachZehnderModulator(Processor):
    ...
    :math:`m` is estimated from the whole array (batch rows pooled) and
    one laser draw is made per array element.
