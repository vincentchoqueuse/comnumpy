...
seg = 5 # number of fiber segments
...
step_MIMO_list = np.logspace(-6, -2, num = 10)
...
chain = Sequential([
            ...
            recorder_before_CMA,
            # Inserare SOPTracker (compensare SOP la rata de simbol, aplicată tuturor eșantioanelor)
...
df.to_csv(f"dates_{seg}_seg_PMD_PDL_CMA_16qam.csv", index=False)

plt.figure(figsize=(7, 5))
plt.loglog(step_MIMO_list, avg_ber_per_step, marker='o', linewidth=2)

plt.xlabel("CMA step (μ)")
