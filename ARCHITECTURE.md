...
### 4.18 Portée d'un estimateur : partagée ou par trajet (nouveau)

| # | Décision | Motif | Alternatives rejetées | Statut |
|---|----------|-------|-----------------------|--------|
| D49 | **Tout estimateur déclare si sa grandeur est partagée par les trajets ou propre à chacun**, et se comporte en conséquence. *Partagée* — décalage de fréquence porteuse : un laser bat contre un oscillateur local, il y a **un** nombre, estimé **conjointement** sur tous les trajets à la fois. *Par trajet* — déséquilibre IQ (un récepteur), offset DC (un convertisseur), rotation résiduelle (une sortie du papillon) : une estimation chacun. Les blocs non généralisés appellent `validate_single_path`, qui refuse un signal multi-trajets par un message D38 nommant la grandeur mesurée et les deux conduites possibles. Et la règle du **scalaire en entrée, scalaire en sortie** s'applique aux grandeurs estimées : `theta_` est un `float` pour un trajet, un tableau sinon | Tant que les signaux étaient unidimensionnels la question ne se posait pas. Dès que la chaîne porte deux polarisations — ce que D47 vient de rendre possible — **laisser numpy diffuser tranche la question par accident**, et l'audit montre qu'il la tranchait faux dans les deux sens. **Deux défauts trouvés en la posant.** (i) `BlindIQCompensator` sur un `(2, N)` empilait parties réelle et imaginaire des *deux* polarisations en une matrice 4×N, gardait le bloc 2×2 supérieur gauche de la covariance et rendait un signal **pire que celui qu'on lui donnait** — `var(I)/var(Q) = 2499` au lieu de 1 — **sans lever d'erreur**. C'est la faute la plus chère du lot : une réponse fausse en silence. (ii) `DCCorrector` ne diffusait pas son **propre** axe documenté : la moyenne d'un `(P, N)` selon `axis=-1` revenait en `(P,)` et la soustraction levait une erreur de forme — le bloc était inutilisable au-delà de la dimension 1, quel que soit le contexte. Sur l'axe partagé, le gain n'est pas que de propreté : l'estimation conjointe du CFO voit deux fois plus de données, et le test le mesure — erreur moyenne plus faible que sur une polarisation seule | Faire diffuser silencieusement (donne la réponse « par trajet » à tous les blocs, y compris ceux où elle est fausse, et c'est exactement le défaut IQ ci-dessus) ; un argument `joint=True` par bloc (la portée est une propriété de la grandeur physique, pas un réglage : le CFO n'est jamais par trajet, le déséquilibre IQ jamais partagé) ; généraliser tous les estimateurs d'un coup (la famille *data-aided* est ajustée contre **une** séquence de référence, donc un signal multi-trajets y est ambigu **par construction** — le garde-fou est la bonne réponse, pas une généralisation qui devrait inventer une convention) ; laisser `theta_` toujours en tableau (casse la lecture du cas mono-trajet, que toute la bibliothèque traite en scalaire) | **Acté** |
...
5ter. **D35** — *si et seulement si* le déclencheur est atteint (3ᵉ script
   `validation/` au squelette identique) : extraction de `monte_carlo()` depuis
...
- *Format de scénario sérialisé* — extension naturelle de D31 + D35 : un
  ...
  graine (D6) et métriques, exécutable par `monte_carlo()` sans code. Utile
